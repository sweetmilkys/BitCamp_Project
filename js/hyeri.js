...
hyeri.page={
    ...
    a:()=>{
		...
		$('<div/>').attr({id:"bir_wrap",style:"diplay:table; width:100%; height:34px; text-align: center;"}).appendTo($('#add_form_middle'));
		
		
		$('<div/>').attr({class:"bir_yy",id:"bir_yy_d"}).appendTo($('#bir_wrap'));
		$('<span/>').addClass('bir_box').attr({id:"bir_yy_s"}).appendTo($('#bir_yy_d'));
		$('<select/>').addClass('bir_sel ').attr({id:"bir_year",title:"년", style: '-webkit-appearance: menulist;'}).appendTo($('#bir_yy_s'));
		...
		$('<select/>').addClass('bir_sel ').attr({id:"bir_month",title:"월", style: '-webkit-appearance: menulist;'}).appendTo($('#bir_mm_s'));
		...
		$('<select/>').addClass('bir_sel').attr({id:"bir_day",title:"일", style: '-webkit-appearance: menulist;'}).appendTo($('#bir_dd_s'));
		...
		$('<select/>').addClass('gen_sel ').attr({id:"gender",title:"성별", style: '-webkit-appearance: menulist;'}).appendTo($('#gender_wrap'));
    ...
    l:()=>{
		...
		$('#content').html('<div class="je_sign-in-form" >'
				...
				+ '<input placeholder="관리자: admin // 사용자: test" autofocus="autofocus" class="je_sign-in-form__form__input form-control sign-in-form__form__email" type="text" name="user[email]" id="user_email">'
				+ '<div id="h_rs" class="je_sign-in-form__form__input-wrap je_sign-in-form__form__password">'
				+ '<input placeholder="1234" autocomplete="off" class="je_sign-in-form__form__input form-control" type="password" name="user[password]" id="user_password">'
				+ '</div>'
		...
		$($('#user_email')).keydown( a => {if (a.keyCode == 13) $('#login_btn1').trigger('click')});
		$($('#user_password')).keydown( a => {if (a.keyCode == 13) $('#login_btn1').trigger('click')});
		$('#join_btn1').click(e=>{
			e.preventDefault();
			hyeri.page.a();
		});
